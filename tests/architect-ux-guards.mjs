...
assert.match(architect, /recovery_code:/);
...
assert.doesNotMatch(index, /SELECT .*token_hash.*architect_auth_state[\s\S]{0,200}return json\([^)]*token_hash/);
...
assert.match(index, /blocking_reason/);
assert.match(index, /lmstudio_model_not_loaded/);
assert.match(architect, /projectBlockerText/);
assert.match(architect, /Ожидание:/);
