...
const localSetup = fs.readFileSync("agent/setup_windows_local.py", "utf8");
const localInstaller = fs.readFileSync("agent/Install Windows Node.cmd", "utf8");
const serviceInstaller = fs.readFileSync("agent/Install Windows Service.cmd", "utf8");
const windowsPackageBuilder = fs.readFileSync("scripts/build_fixed_agent_package.py", "utf8");
...
need(localSetup.includes('VERSION = "0.3.16"'), "local Windows installer version missing");
need(localSetup.includes("LOCALAPPDATA"), "local Windows installer is not user-scoped");
need(localSetup.includes("python-runtime"), "local Windows installer does not use bundled runtime");
need(localSetup.includes("No Administrator permission or system Python was used."), "local install success marker missing");
need(!localSetup.toLowerCase().includes("powershell"), "local Windows installer must not invoke PowerShell");
need(!localSetup.toLowerCase().includes("winget"), "local Windows installer must not invoke Winget");
need(!localInstaller.toLowerCase().includes("powershell"), "default Windows installer must not invoke PowerShell");
need(localInstaller.includes("setup_windows_local.py"), "default Windows installer does not select Local Agent");
need(serviceInstaller.toLowerCase().includes("powershell.exe"), "explicit Windows Service installer missing");
need(windowsPackageBuilder.includes("python-3.14.7-embeddable-amd64.zip"), "bundled AMD64 Python runtime missing");
need(windowsPackageBuilder.includes("python-3.14.7-embeddable-win32.zip"), "bundled x86 Python runtime missing");
need(windowsPackageBuilder.includes("python-3.14.7-embeddable-arm64.zip"), "bundled ARM64 Python runtime missing");
...
need(agentV1.includes("install_lmstudio_windows_native"), "Windows native llmster installer missing");
need(agentV1.includes("lmstudio_sha512_checksum_unavailable"), "llmster checksum requirement missing");
need(agentV1.includes("lmstudio_sha512_mismatch"), "llmster checksum mismatch guard missing");
need(agentV1.includes("LMS_BOOTSTRAP_INSTALL_SH"), "llmster fixed bootstrap mode missing");
