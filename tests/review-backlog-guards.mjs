...
need(setup.includes('start= delayed-auto'), "Windows service delayed-auto start missing");
need(setup.includes('NT AUTHORITY\\LocalService'), "Windows service LocalService account missing");
need(setup.includes('failure $ServiceName'), "Windows service recovery policy missing");
...
need(setup.includes('Removed legacy Startup shortcut'), "legacy Startup migration missing");
...
need(serviceHost.includes('CITADEL_SERVICE_MANAGED'), "service host does not mark managed child");
