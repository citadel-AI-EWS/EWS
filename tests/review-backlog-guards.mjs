...
const nodeApi = fs.readFileSync("NODE_API.md", "utf8");
...
need(agentV1.includes("x-node-request-id") && agentV1.includes("uuid.uuid4()"), "agent request nonce signing missing");
need(index.includes("node_request_nonces") && index.includes("replayed_request"), "Controller replay protection missing");
need(index.includes('versionAtLeast(node.agent_version, "0.3.10")'), "staged nonce rollout guard missing");
need(deployWorkflow.includes("wrangler d1 migrations apply citadel-control --remote"), "remote D1 migration gate missing");
need(nodeApi.includes("x-node-request-id") && nodeApi.includes("REQUEST_ID"), "node signature documentation missing request ID");
need(packageJson.includes("presence-storage.mjs") && packageJson.includes("hub-five-questions.mjs"), "npm test omits active JS suites");
