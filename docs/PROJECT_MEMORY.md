...
## 2026-09-26 — Weekly engineering control and telemetry retention

- Baseline checked: `main` at `f98f656a65862249bc5c788d281f371057d014fe`; checked-in signed release is agent 0.3.16. Main CI 36245634236 passed all Linux, Windows, package-integrity and aggregate gates.
- Release blocker: Cloudflare TEST runs 36245433309 and 36245734928 failed before deployment because the optional `OPENROUTER_API_KEY` repository/environment secret remained invalid after safe whitespace, prefix and quote normalization. No secret value was printed or stored. Synthetic runs were consequently skipped. Rollback is not applicable because these commits were never deployed.
- Live state: the last verified TEST deployment is commit `394c6bf76bae57f994b6d09eec4033b78741bc61` (run 36240477221). Its deployment smoke passed with nine registered nodes, but project execution reported unavailable and zero AI/Python-ready workers. A later independent smoke on 2026-09-26 failed with `hub_d1_daily_read_limit_exceeded`; site↔agent and privileged Fleet actions therefore are not live-verified.
- AEGIS Fleet source review: Infrastructure / Fleet remains in Hub; privileged CPU, RAM, agent version, hostname/computer identity and LM Studio readiness are rendered from Controller records, and Health queues the allow-listed signed `system_inventory` mission. The published root Operations console had regressed by exposing a Tailscale IP row; this branch removes that Tailscale-specific UI and adds a guard against its return. No simulated Fleet source was found. Actual privileged command completion could not be exercised while D1 was exhausted.
- CodeRabbit PR133 finding confirmed on current main: seven-day telemetry cleanup was gated by a node-controlled event ID suffix. Change removes that gate and adds a server-controlled hourly Worker cron using the existing indexed `pruneExpiredTelemetry` helper. Functional test proves scheduled deletion of expired rows. Rollback: revert the scoped cron/worker/ingestion/test/checksum commit.
- Local verification before PR: npm dependency audit reported zero vulnerabilities; seven Node test suites passed; Windows/Linux 0.3.16 package builders and agent hashes passed; pip-audit reported no known agent dependency vulnerabilities. Full validate/build/Wrangler checks are repeated on the final branch.
- Unreleased review backlog: PR122 has one CodeRabbit High security finding and a separate atomicity finding; PR119, PR120, PR125, PR127, PR136 also retain unresolved review items and are not part of main. They must not be represented as released. Corresponding Gmail messages remain in Inbox until their complete PR, CI, deployment and live verification lifecycle finishes.
