...
Нужен только Python 3.11+. Адрес контроллера уже встроен. При установке узел сам регистрируется на хабе, получает постоянный номер и появляется в панели; вводить код, логин или подтверждение не нужно.
