...
DIST = ROOT / "dist"
PACKAGE_NAME = "CITADEL_FIXED_AGENT_0.3.17_2026-09-26"
...
def patch_v1(path: Path) -> None:
    ...
    text = must_replace(text, 'VERSION = "0.3.0"', 'VERSION = "0.3.17"', "v1 version")
    text = must_replace(
        text,
...
def patch_v2(path: Path) -> None:
    text = path.read_text(encoding="utf-8")
    text = must_replace(text, 'VERSION = "0.3.0"', 'VERSION = "0.3.17"', "v2 version")
    path.write_text(text, encoding="utf-8", newline="\n")
...
def patch_setup(path: Path, v1_hash: str, v2_hash: str) -> None:
    ...
    text = text.replace('agent_version = "0.3.0"', 'agent_version = "0.3.17"')
    if text == original:
...
def write_extras() -> None:
    ...
    readme = (
        "CITADEL/EWS — исправленный самодостаточный пакет 0.3.17\n\n"
...
def build() -> Path:
    ...
    if 'VERSION = "0.3.17"' not in v1_path.read_text(encoding="utf-8"):
        raise RuntimeError("repository v1 source is not release 0.3.17")
    if 'VERSION = "0.3.17"' not in v2_path.read_text(encoding="utf-8"):
        raise RuntimeError("repository v2 source is not release 0.3.17")
    ...
    if '$ReleaseVersion = "0.3.17"' not in setup_text:
        raise RuntimeError("setup_windows.ps1 release version is not 0.3.17")
    write_extras()
